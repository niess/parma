...
static std::string PREFIX = "input";

extern "C" void set_prefix(const char * prefix) {
    PREFIX = prefix;
}
...
if(ifirst == 0) { // Initialization
 ...
 for(i=0;i<=7;i++) {
  if(i<=2) { // neutron, proton alpha
   dname=PREFIX+"/"+pname[i]+"/solar-dep.inp";
  } else if(i==3) {
   dname=PREFIX+"/"+pname[i]+"/solar-dep-EL.inp";
  } else if(i==4) {
   dname=PREFIX+"/"+pname[i]+"/solar-dep-PO.inp";
  } else if(i==5) {
   dname=PREFIX+"/"+pname[i]+"/solar-dep-PH.inp";
  } else if(i==6) {
   dname=PREFIX+"/"+pname[i]+"/solar-dep.plus";
  } else if(i==7) {
   dname=PREFIX+"/"+pname[i]+"/solar-dep.mins";
  }
 ...
 dname=PREFIX+"/ions/solar-dep.inp";
...
if(ifirst == 0) { // Initialization
 ifirst = 1;
 dname=PREFIX+"/neutro/correction-depth-rigid.inp";
 ifstream ifs(dname,ios::in);
 getline(ifs, str); // Kara-yomi
...
if(p[1] == 0) { // Initialization
 dname=PREFIX+"/neutro/Geo-Dep.inp";
 ifstream ifs1(dname,ios::in);
 getline(ifs1, str); // Kara-yomi
 getline(ifs1, str); // Read p(1)-p(3)
 istringstream s1(str);
 s1 >> p[1] >> p[2] >> p[3];
 dname=PREFIX+"/neutro/Water-Dep.inp";
 ...
 dname=PREFIX+"/neutro/Aircraft-Dep.inp";
...
if(B[2] == 0.0) { // first time
 dname=PREFIX+"/neutro/Depth-Dep-mid.out"; 
 ifstream ifs(dname,ios::in);
 getline(ifs, str); // Kara-yomi
 getline(ifs, str); // Kara-yomi
 getline(ifs, str);
 ...
 dname=PREFIX+"/neutro/Rigid-Dep.inp";
 ifstream ifs1(dname,ios::in);
 getline(ifs1, str); // Kara-yomi
 getline(ifs1, str); // Kara-yomi
 getline(ifs1, str); // Kara-yomi
...
if(B[4] == 0.0) { // first time
 dname=PREFIX+"/neutro/Depth-Dep-hig.out"; 
 ifstream ifs(dname,ios::in);
 getline(ifs, str); // Kara-yomi
 getline(ifs, str); // Kara-yomi
 getline(ifs, str);
...
if(ifirst==0) { // Initialization
 ...
 for(i=0;i<=npart;i++) {
  if(i==0) {
   dname=PREFIX+"/neutro/bestR.inp";
  } else {
   dname=PREFIX+"/elemag/bestR-"+pname[i]+".inp";
  }
  ifstream ifs(dname,ios::in);
  getline(ifs, str); // Kara-yomi
  for(id=1;id<=ndep;id++) {
...
if(ifirst == 0) { // Initialization
 ...
 for(i=0; i<=5; i++) {
  if(i <= 2) {
   dname=PREFIX+"/"+pname[i]+"/Rigid-Dep.inp"; // neutron, proton, alpha
  } else if(i == 3) {
   dname=PREFIX+"/"+pname[i]+"/Rigid-Dep-EL.inp"; // electron
  } else if(i == 4) {
   dname=PREFIX+"/"+pname[i]+"/Rigid-Dep-PO.inp"; // positron
  } else if(i == 5) {
   dname=PREFIX+"/"+pname[i]+"/Rigid-Dep-PH.inp"; // photon
  }
  ifstream ifs(dname,ios::in);
  getline(ifs, str); // Kara-yomi
  for (ib=1; ib<=nBdata; ib++) {
 ...
 dname=PREFIX+"/ions/Rigid-Dep.inp";
 ifstream ifs1(dname,ios::in);
 for(i=npart-5;i<=npart;i++) {
  getline(ifs1, str);
  for(ib=1;ib<=nBdata;ib++) {
...
//      Read A parameter
 dname=PREFIX+"/neutro/fitting-lowspec.inp"; 
 ifstream ifs(dname,ios::in);
 getline(ifs, str); // Kara-yomi
 getline(ifs, str);
 istringstream s(str);
...
if(down==0.0) { // Initialization
 ...
 for(ig=1;ig<=ngroup;ig++) {
  dname=PREFIX+"/ions/primary-"+gname[ig]+".inp";
  ifstream ifs(dname,ios::in);
  getline(ifs, str); // Kara-yomi
  for(id=1;id<=ndep;id++) {
   getline(ifs, str);
 ...
 dname=PREFIX+"/ions/dEdx-table.inp";
 ifstream ifs1(dname,ios::in);
 getline(ifs1, str); // Kara-yomi
 getline(ifs1, str); // Kara-yomi
 getline(ifs1, str);
...
if(ifirst ==0) { // Initialization
 ifirst = 1;
 for(i=1;i<=5;i++) {
  if(i==1) {dname=PREFIX+"/proton/fitting-lowspec.inp";}
  else if(i==2) {dname=PREFIX+"/alphaa/fitting-lowspec.inp";}
  else if(i==3) {dname=PREFIX+"/elemag/fitting-lowspec-EL.inp";}
  else if(i==4) {dname=PREFIX+"/elemag/fitting-lowspec-PO.inp";}
  else if(i==5) {dname=PREFIX+"/elemag/fitting-lowspec-PH.inp";}
  ifstream ifs(dname,ios::in);
  getline(ifs, str); // Kara-yomi
  for(id=1;id<=ndep;id++) {
   getline(ifs, str);
 ...
 dname=PREFIX+"/ions/fitting-lowspec.inp";
 ifstream ifs1(dname,ios::in);
 getline(ifs1, str); // Kara-yomi
 for(i=npart-5;i<=npart;i++) {
  getline(ifs1, str);
...
if(ifirst == 0) { // Initialization
 ...
 for(ip2=1;ip2<=npart;ip2++) {
  dname=PREFIX+"/muon--/final135."+charge[ip2];
  ifstream ifs1(dname,ios::in);
  getline(ifs1, str); // Kara-yomi
  ...
  dname=PREFIX+"/muon--/final2467."+charge[ip2];
  ifstream ifs2(dname,ios::in);
  getline(ifs2, str); // Kara-yomi
  for(id2=1;id2<=ndep;id2++) {
   getline(ifs2, str);
...
// *******************************************************
extern "C" double getSpecCpp(int ip, double s,double r,double d,double e,double g)
// ip: particle ID
// s: W index
...
if(ifirst == 0) { // Initialization
 ifirst = 1;
 dname=PREFIX+"/angle/NeutronGround.out"; 
 ifstream ifs(dname,ios::in);
 for(i=1;i<=maxfit;i++) {
  getline(ifs, str);
  istringstream s(str);
...
if(ifirst == 0) { // Initialization
 ifirst = 1;
 for(ip2=1;ip2<=npart;ip2++) {
  dname=PREFIX+"/angle/BkH-"+pname[ip2]+".inp";
  ifstream ifs(dname,ios::in);
  getline(ifs, str); // Kara-yomi
  for(i=1;i<=nBHpara;i++) {
   getline(ifs, str);
...
if(ifirst == 0) { // Initialization
 ...
 for(ip1=1;ip1<=npart;ip1++) {
  dname=PREFIX+"/angle/"+pname[ip1]+".out";
  ifstream ifs1(dname,ios::in);
  getline(ifs1, str); // Kara-yomi
  for(i=1;i<=maxfit;i++) {
   for(is=1;is<=nsur;is++) {
  ...
  dname=PREFIX+"/angle/"+pname[ip1]+"-Eint.out";
  ifstream ifs2(dname,ios::in);
  getline(ifs2, str); // Kara-yomi
  for(is=1;is<=nsur;is++) {
   for(ic=1;ic<=ncor;ic++) {
...
// *******************************************************
extern "C" double getSpecAngFinalCpp(int ip,double s,double r,double d,double e,double g,double ang)
...
// ************************************************
extern "C" double getHPcpp(int iy0,int im0,int id0)
// ************************************************
{
const int nmonth=12;
const int nday=31;
...
if(ifirst == 0) {  // Initialization
 ifirst=1;
 dname=PREFIX+"/FFPtable.day"; // Read daily force potential
 ifstream ifs(dname,ios::in);
 getline(ifs, str);
 istringstream s(str);
 s >> iystart >> iyend;
 ...
 dname=PREFIX+"/FFPtable.uso"; // Read annual force field potential
 ifstream ifs1(dname,ios::in);
 getline(ifs1, str);
 istringstream s1(str);
 s1 >> iysUs >> iyeUs;
...
// ******************************************************
extern "C" double getrcpp(double cido,double ckei) // cido and ckei are center of ido&keido of each grid
// ******************************************************
{
double getr;
...
if(ifirst == 0) {  // Initialization
 ifirst=1;
 dname=PREFIX+"/CORdata.inp"; // Read vertical cut-off rigidity database
 ifstream ifs(dname,ios::in);
 getline(ifs, str);
 istringstream s(str);
...
// ******************************************************
extern "C" double getdcpp(double alti, double cido) // getd in g/cm^2, alti in km
// ******************************************************
{
double getd;
...
if(ifirst == 0) {  // Initialization
 ifirst=1;
 dname=PREFIX+"/AtomDepth.inp"; // Read vertical cut-off rigidity database
 ifstream ifs(dname,ios::in);
 getline(ifs, str); // kara-yomi
...
if(ifirst == 0) { // Initialization
 ifirst = 1;
 dname=PREFIX+"/elemag/flux511keV.inp";
 ifstream ifs(dname,ios::in);
 getline(ifs, str); // Kara-yomi
 for(id=1;id<=ndep;id++) {
  getline(ifs, str); // read data
